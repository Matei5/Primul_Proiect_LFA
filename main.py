...
for i in range(nrM):
    ...
    if inputs[0] in muchii:
        if inputs[1] in muchii[inputs[0]]:
            if inputs[2] not in muchii[inputs[0]][inputs[1]]:
...
for stare1 in muchii.keys():
    if "." in muchii[stare1]:

        maiSuntLambdas = 1
        while maiSuntLambdas:
            maiSuntLambdas = 0

            muchii[stare1]["am pasit in starile"] = [stare1]
            for starePrinLambda in muchii[stare1]["."]:
                if starePrinLambda in muchii[stare1]["am pasit in starile"]:
                    continue
                muchii[stare1]["am pasit in starile"] += [starePrinLambda]

                if starePrinLambda in muchii.keys():
                    for key in muchii[starePrinLambda]:
                        if key not in muchii[stare1]:
                            muchii[stare1][key] = muchii[starePrinLambda][key]
                        else:
                            if key == "." :
                                maiSuntLambdas = 1
                            muchii[stare1][key] += muchii[starePrinLambda][key]
                            muchii[stare1][key] = list(set(muchii[stare1][key]))
